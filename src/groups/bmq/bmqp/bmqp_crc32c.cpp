...
//                       https://github.com/bloomberg/comdb2/tree/main/crc32c
//
// We compared it to a hardware-based implementation for calculating CRC-32C in
...
// Compiler-specific
    ...
    (defined(BSLS_PLATFORM_CPU_X86) || defined(BSLS_PLATFORM_CPU_X86_64)) &&  \
    (defined(__SSE4_2__) && __SSE4_2__)
#define BMQP_CRC32C_LIKE_X86_GCC
#endif
...
const char k_LOG_CATEGORY[] = "BMQP.CRC32C";

/// A global CRC32-C calculator function used by the default implementation
/// class methods (`calculate()`).  Set by `initialize()` to a
                                          ...
                                          unsigned int         crc)
{
    ...
    while (data < end) {
        ...
        u32b = (data[0] << 0) | (data[1] << 8) | (data[2] << 16) |
               (data[3] << 24);
        data += 4;
        ...
        term2 = u32b >> 16;
        crc   = crc ^ term1 ^ k_CRC_TABLE_IL8_O40[term2 & 0x000000FF] ^
...
#define C(i)                                                                  \
    ...
    tmp ^= static_cast<bsls::Types::Uint64>(k_MUL_TABLE1_336[(c2 >> 8) & 0xFF])
           << 8;
    tmp ^=
        static_cast<bsls::Types::Uint64>(k_MUL_TABLE1_336[(c2 >> 16) & 0xFF])
        << 16;
    tmp ^=
        static_cast<bsls::Types::Uint64>(k_MUL_TABLE1_336[(c2 >> 24) & 0xFF])
        << 24;

    // merge in c1
    tmp ^= k_MUL_TABLE1_672[c1 & 0xFF];
    tmp ^= static_cast<bsls::Types::Uint64>(k_MUL_TABLE1_672[(c1 >> 8) & 0xFF])
           << 8;
    tmp ^=
        static_cast<bsls::Types::Uint64>(k_MUL_TABLE1_672[(c1 >> 16) & 0xFF])
        << 16;
    tmp ^=
        static_cast<bsls::Types::Uint64>(k_MUL_TABLE1_672[(c1 >> 24) & 0xFF])
        << 24;
                                                ...
                                                unsigned int         crc)
{
    ...
    if (mask != 0) {
        bsls::Types::Uint64 limit = bsl::min(
            static_cast<bsls::Types::Uint64>(length),
    ...
    while (offset + sizeof(bsls::Types::Uint64) <= length) {
        ...
        offset += sizeof(bsls::Types::Uint64);
    }
...
#endif  // BMQP_CRC32C_LIKE_X86_GCC
...
#ifdef BSLS_PLATFORM_CPU_64_BIT
            BALL_LOG_INFO << "Using hardware version for CRC32-C computation "
                             "(SSE4.2 instructions available, 64-bit mode)";

            g_crc32cCalculator = crc32cSse64bit;

#else
            BALL_LOG_INFO << "Using hardware version (serial) for CRC32-C "
                             "computation (SSE4.2 instructions available, "
                             "32-bit mode)";

            g_crc32cCalculator = crc32cHardwareSerial;
#endif  // BSLS_PLATFORM_CPU_64_BIT
#undef BMQP_SSE4_2
        }
        else {
            BALL_LOG_INFO << "Using software version for CRC32-C computation "
                             "(SSE4.2 instructions not available)";

            g_crc32cCalculator = crc32cSoftware;
        }
                                            ...
                                            unsigned int       crc)
{
    ...
    for (int i = 0; i < (numBuffers - 1); ++i) {
        ...
        crc = calculateSoftware(buffer.data(), buffer.size(), crc);
    }

    // Handle last data buffer
    crc = calculateSoftware(blob.buffer(numBuffers - 1).data(),
                            blob.lastDataBufferLength(),
                            crc);
